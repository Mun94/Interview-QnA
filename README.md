...
해결 방법으로는 각각 브라우저에 맞는 코드를 설정하는 방법과 jQuery, underscore.js와 같은 라이브러리들이 제공하는 함수를 활용하는 방법이 있다.   

### Q 자바스크립트의 단점   
- 브라우저상에 소스가 노출되어서 보안에 취약하며 신경을 많이 써야 한다.   
- js는 굉장히 유연하고 뛰어난 표현력을 가지고 있지만 느슨한 타입 체크로 인해 오류가 자주 발생한다.
...
### Q let, const, var
var은 스코프(해당 값을 사용할 수 있는 영역)가 함수 단위 이고 const, let은 블록단위 이다.
